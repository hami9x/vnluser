...
public class FileUtils {
	...
	public static HttpOutputResource readHttpOutputResource(String uri) throws java.io.IOException {
		String fullpath = FileUtils.getRuntimeFolderPath() + uri.replace("/", File.separator);
		if(!uri.startsWith("/")){
			fullpath = FileUtils.getRuntimeFolderPath() + File.separator + uri.replace("/", File.separator);
		}		
		...
		if(file.isFile()){			
		   ...
		   // System.out.println(fullpath +" length: "+data.length);
		    ByteBuf byteBuf = Unpooled.copiedBuffer(data);					
			//System.out.println(new String(data));			
			return new HttpOutputResource(byteBuf, file.lastModified() );	
		}			
		throw new IllegalArgumentException(uri + " not existed !");
	}
